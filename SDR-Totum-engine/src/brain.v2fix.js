...
const llmProvider = require('./llm_provider'); // camada agnostica (gemini|nvidia)
...
function simApiKey() {
  return process.env.GEMINI_API_KEY_SIM || ''; // slot 2a chave (sim) — inerte se vazio
}
...
function authorizeActions(stage, raw) {
  ...
  const booked = Boolean(raw.booked) && (stage === 'agendamento' || stage === 'encerrado');
...
OBJEÇÕES: se o lead levantar objeção real (${obj ? obj.categories.join(', ') : 'preço, tempo, já tem, não preciso, genérica'}), ACOLHA (nunca corrija), reintroduza como curiosidade, e devolva o campo "objecao" com a categoria. Preço: nunca revele valor, redirecione pra prévia/página. Se o lead VOLTAR a engajar depois da objeção, RETOME o fluxo do ponto de origem e avance normalmente — não fique preso só acolhendo. ANTI-LOOP (crítico): só preencha "objecao" se a ÚLTIMA mensagem do lead, sozinha, trouxer uma objeção NOVA e explícita. Se o lead respondeu de forma neutra ou cooperativa (confirmou ser o decisor, respondeu uma pergunta, disse "ta"/"ok"/"pode seguir"/"sou eu"), então "objecao": null — a objeção anterior JÁ foi acolhida, não a reconte. Nesse caso RETOME: avance um estágio e faça uma PERGUNTA DE IMPLICAÇÃO que ligue a dor ao custo de não agir (ex.: quantas avaliações/pacientes ele perde por mês com isso sem resolver), sempre sem revelar preço.

REGRAS DE AVANÇO (CRÍTICO): leia o HISTÓRICO + a ÚLTIMA mensagem. Se a condição "${st.advance_when || ''}" JÁ foi atendida — mesmo em resposta curta/seca (ex.: "site não", "sou eu", "pode seguir") — então AVANCE: stage_proposto = "${st.next || stageId}" e NÃO repita a pergunta já respondida. Só mantenha "${stageId}" se a condição realmente NÃO foi atendida. Avance no máximo 1 passo; nunca pule etapas.
...
{
  "reply": ["msg curta 1", "msg curta 2 (opcional)"],
...
async function generate(prompt, kind = 'prod') {
  return llmProvider.generate(prompt, kind); // gemini (default) ou nvidia, por env
}
...
async function callBrain({ session, history = [], lastMessage, classificacao = '', flowOverride = null }) {
  const _kind = flowOverride ? 'sim' : 'prod'; // sim usa provider/chave dedicada (llm_provider)
  ...
  try { raw = extractJson(await generate(prompt, _kind)); }
    ...
    console.error('[brain.v2fix] generate erro:', e && e.message);
    return {
  ...
  const reply = Array.isArray(raw.reply) ? raw.reply.map(String).filter(Boolean).slice(0, 3) : [];
